...
% initialization
CMLE = cell(R, 1); SECMLE = CMLE ; BETAS = CMLE; ZIJ = CMLE; BETAS_REAL = CMLE;
% Replications
parfor r=1:R, [CMLE{r} SECMLE{r} BETAS{r} ZIJ{r} BETAS_REAL{r}] = Simulation(n, beta_11, beta_12, beta_21, beta_22, beta_23); end
% prepare screem output
%TCMLE = (CMLE-ones(R,1)*theta)./SECMLE; aCMLE = abs(TCMLE)>=1.96;
%OUTPUT = [CMLE SECMLE TCMLE aCMLE];
OUTPUT = [CMLE SECMLE BETAS ZIJ BETAS_REAL]
save('Simulations_Jochmans_Hybrid_Design1_n25.mat','OUTPUT')

function [cmle se_cmle betas zij_real_estimated betas_real] = Simulation(n, beta_11, beta_12, beta_21, beta_22, beta_23)
warning('off','all'); % prevent overparametrization warning for MLE

%%%% Data generation % see paper for details on design choices %%%%

% generating FE
% keep this part to just keep track of indices
a = repmat(1:(n*n),1)';
...
%%%% Transforming data for second stage estimation %%%%
[monte_dgp_subset_subset] = TransformData(monte_dgp,combinations,ss,n);

%%%% Estimating the second stage %%%%

[betas zij_real_estimated betas_real] = SecondStage(monte_dgp_subset_subset,cmle,beta_21,beta_22,beta_23);

function [monte_dgp_subset_subset] = TransformData(monte_dgp,combinations,s,n)
%removing missings in x2
monte_dgp_subset = monte_dgp;
monte_dgp_subset(any(ismissing(monte_dgp_subset(:,5)),2), :) = []; 
...
% as we do not need FE, this part is not needed
...
function [betas zij_real_estimated betas_real] = SecondStage(monte_dgp_subset_subset,cmle,beta_21,beta_22,beta_23);
% taking only combinations used in Jochmans
monte_dgp_secondstage = monte_dgp_subset_subset;

% create variable zij and getting inverted mills ratio
monte_dgp_secondstage_zij = cmle(1)*monte_dgp_secondstage(:,4) + cmle(2)*monte_dgp_secondstage(:,5) + cmle(3)*monte_dgp_secondstage(:,7);
...
zij_real = beta_21*monte_dgp_secondstage(:,4) + beta_22*monte_dgp_secondstage(:,5) + beta_23*monte_dgp_secondstage(:,7);
...
table_secondstage = array2table([monte_dgp_secondstage(:,4),monte_dgp_secondstage(:,5),monte_dgp_secondstage_mills,monte_dgp_secondstage(:,3)]);
second_stage = fitlm(table_secondstage,'Intercept',false);
betas = table2array(second_stage.Coefficients(1:3,1));

table_secondstage_real = array2table([monte_dgp_secondstage(:,4),monte_dgp_secondstage(:,5),secondstage_mills_real,monte_dgp_secondstage(:,3)]);
