...
% parameters
R  = 100; % # of Monte Carlo replications
n  =   25; % # of nodes 
...
% initialization
CMLE = cell(R, 1); SECMLE = CMLE ; BETAS = CMLE;  FE_SELECTION = CMLE; ZIJ = CMLE; fe_xi_i_zeta_j_comb=CMLE; monte_dgp_secondstage=CMLE;%SE_X = CMLE;
% Replications
parfor r=1:R, [CMLE{r} SECMLE{r} BETAS{r} FE_SELECTION{r} ZIJ{r} fe_xi_i_zeta_j_comb{r} monte_dgp_secondstage{r}] = Simulation(n); end
% prepare screem output
%TCMLE = (CMLE-ones(R,1)*theta)./SECMLE; aCMLE = abs(TCMLE)>=1.96;
%OUTPUT = [CMLE SECMLE TCMLE aCMLE];
OUTPUT = [CMLE SECMLE BETAS FE_SELECTION ZIJ fe_xi_i_zeta_j_comb monte_dgp_secondstage]
save('Simulations_Jochmans_Hybrid.mat','OUTPUT')

function [cmle se_cmle betas FE_selection_real_estimated zij_real_estimated fe_xi_i_zeta_j_comb monte_dgp_secondstage] = Simulation(n)
...
[fe_xi_i_zeta_j_est monte_dgp_subset condition_all fe_xi_i_zeta_j_comb] = FEHybridApproach(monte_dgp,combinations,ss,n,cmle);
FE_selection_real_estimated = [repmat(1:(n+n),1)', fixed_effects_selection, fe_xi_i_zeta_j_est];

%%%% Estimating the second stage %%%%

[betas zij_real_estimated monte_dgp_secondstage] = SecondStage(monte_dgp_subset,condition_all,fe_xi_i_zeta_j_est,cmle,fixed_effects_selection,beta_21,beta_22,beta_23);

function [fe_xi_i_zeta_j_est monte_dgp_subset condition_all fe_xi_i_zeta_j_comb] = FEHybridApproach(monte_dgp,combinations,s,n,cmle)
...
combinations_subset = [combinations_subset_i1, combinations_subset_j1];
combinations_subset =  unique(combinations_subset,'rows');
...
while condition_while == 1,
    ...
    for i=1:size(fe_xi_subset,1),
        Nsum_xi(i) = sum(monte_dgp_subset_subset(monte_dgp_subset_subset(:,1)==fe_xi_subset(i),8));
    end
    Ncondition1_xi = (Nsum_xi'==Ncount_xi);
    Ncondition2_xi = (Nsum_xi'==0);
    index_delete1_xi = find(Ncondition1_xi == 1);
    index_delete2_xi = find(Ncondition2_xi == 1);
    fe_delete_new_xi = vertcat(fe_xi_subset(index_delete1_xi),fe_xi_subset(index_delete2_xi));
    ...
    for i=1:size(fe_zeta_subset,1),
        Nsum_zeta(i) = sum(monte_dgp_subset_subset(monte_dgp_subset_subset(:,2)==fe_zeta_subset(i),8));
    end
    Ncondition1_zeta = (Nsum_zeta'==Ncount_zeta);
    Ncondition2_zeta = (Nsum_zeta'==0);
    index_delete1_zeta = find(Ncondition1_zeta == 1);
    index_delete2_zeta = find(Ncondition2_zeta == 1);
    fe_delete_new_zeta = vertcat(fe_zeta_subset(index_delete1_zeta),fe_zeta_subset(index_delete2_zeta));
    ...
    condition_xi = not(ismember(monte_dgp_subset_subset(:,1),fe_delete_xi));
    condition_zeta = not(ismember(monte_dgp_subset_subset(:,2),fe_delete_zeta));
    conditions = condition_xi.*condition_zeta;
...
condition_xi = not(ismember(monte_dgp_subset(:,1),fe_delete_xi));
condition_zeta = not(ismember(monte_dgp_subset(:,2),fe_delete_zeta));
condition_all = condition_xi.*condition_zeta.*monte_dgp_subset_subset_index;

% initial values for log-likelihood
fe_xi_i_zeta_j_est = ones(n+n,1);
fe_xi_i_zeta_j_comb = vertcat(fe_xi_subset,fe_zeta_subset);

% define likelihood
F_logit = @(e) 1./(1+exp(-e));
LikelihoodHybrid = @(fe_xi_i_zeta_j_est) -sum((monte_dgp_subset(:,8).*(log(F_logit(fe_xi_i_zeta_j_est(monte_dgp_subset(:,1))+fe_xi_i_zeta_j_est(monte_dgp_subset(:,2))+cmle(1)*monte_dgp_subset(:,6)+cmle(2)*monte_dgp_subset(:,7)+cmle(3)*monte_dgp_subset(:,11))))+(1-monte_dgp_subset(:,8)).*(log(1-(F_logit(fe_xi_i_zeta_j_est(monte_dgp_subset(:,1))+fe_xi_i_zeta_j_est(monte_dgp_subset(:,2))+cmle(1)*monte_dgp_subset(:,6)+cmle(2)*monte_dgp_subset(:,7)+cmle(3)*monte_dgp_subset(:,11)))))).*condition_all(:)); 
...
function [betas zij_real_estimated monte_dgp_secondstage] = SecondStage(monte_dgp_subset,condition_all,fe_xi_i_zeta_j_est,cmle,fixed_effects_selection,beta_21,beta_22,beta_23)
...
monte_dgp_secondstage_zij = fe_xi_i_zeta_j_est(monte_dgp_secondstage(:,1)) + fe_xi_i_zeta_j_est(monte_dgp_secondstage(:,2)) + cmle(1)*monte_dgp_secondstage(:,6) + cmle(2)*monte_dgp_secondstage(:,7) + cmle(3)*monte_dgp_secondstage(:,11);
...
zij_real = fixed_effects_selection(monte_dgp_secondstage(:,1)) + fixed_effects_selection(monte_dgp_secondstage(:,2)) + beta_21*monte_dgp_secondstage(:,6) + beta_22*monte_dgp_secondstage(:,7) + beta_23*monte_dgp_secondstage(:,11);
...
table_secondstage = [monte_dgp_secondstage(:,6),monte_dgp_secondstage(:,7),monte_dgp_secondstage_mills,D_i,D_j,monte_dgp_secondstage(:,3)];
table_secondstage = array2table(table_secondstage);
